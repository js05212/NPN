...
%
function [c, posterior] = mc_mlp_classify_bayes(M, x0, t0, my, Q0, raw)
rand('seed',11112);
...
if isfield(M, 'dbm') && M.dbm.use % this option is not supported for Bayes' version
    ...
    for i = 1:my.mc_dropout
        ...
        for l = 2:n_layers
            ...
            if l < n_layers 
                ...
                elseif my.use_tanh==2
                    ratio_v = posterioro./sqrt(posterioro_s);
        ...
        if M.output.binary
            ...
            sum_posterior_s = sum_posterior_s+posterioro_s;
        end
    end
end
...
elseif my.regression_target==0
    ...
    c = sum_posterior/my.mc_dropout;
    posterior = sum_posterior_s/my.mc_dropout;
end
