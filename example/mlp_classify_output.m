...
%
%function [c, posterior] = mlp_classify(M, x0, my, Q0, raw)
function mlp_classify_output(id)
...
if isfield(M, 'dbm') && M.dbm.use
    ...
    for l = 2:n_layers
        if M.dropout.use && l > 2
            posterior = bsxfun(@plus, posterior * (M.W{l-1}*(1-my.dropout)), M.biases{l}');
        else
...
x = x0;
m = posterior;
save(sprintf('%s_output.mat',file_id),'x','m');
